...
export default function Testimonials() {
  // Initialize Embla Carousel with responsive options
  const [emblaRef, emblaApi] = useEmblaCarousel({
    loop: true,
    dragFree: true,
    align: "start",
  ...
  // Clip path for the section - keeping as requested
  const sectionClipPath = "polygon(0 5%, 100% 0, 100% 95%, 0 100%)";
  ...
  return (
    <section className="py-24 bg-[#32750a]/10 text-gray-900 relative overflow-hidden" style={{ clipPath: sectionClipPath }}>
      ...
      <div className="container mx-auto px-4 relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeIn} className="text-center mb-16">
          <Badge className="mb-4 bg-[#32750a] hover:bg-[#32750a]/90 px-4 py-1.5 text-base">Team Stories</Badge>
          <h2 className="text-3xl md:text-5xl font-bold mb-6 text-gray-900">Hear From Our Team</h2>
          <p className="text-xl md:text-2xl text-gray-700 max-w-3xl mx-auto">Our employees share their experiences working at Tolbert Garage Door.</p>
          <p className="text-xl md:text-2xl text-gray-700 max-w-3xl mx-auto mt-2">Our Team, Your Assurance</p>
        </motion.div>

        <div className="relative mx-auto">
          {/* Navigation buttons - positioned differently for better mobile experience */}
          <div className="flex justify-between mb-6 md:absolute md:top-1/2 md:transform md:-translate-y-1/2 md:-left-4 md:-right-4 md:z-10">
            <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
              <Button variant="ghost" size="icon" className="bg-white shadow-lg hover:bg-[#32750a]/10 text-[#32750a] rounded-full h-12 w-12" onClick={scrollPrev} disabled={!prevBtnEnabled}>
                <ChevronLeft className="h-6 w-6" />
              </Button>
          ...
          <div className="overflow-hidden" ref={emblaRef}>
            <div className="flex">
              {testimonials.map((testimonial, index) => (
                <div className="flex-[0_0_100%] min-w-0 pl-4 pr-4 md:flex-[0_0_50%]" key={index}>
...
function TestimonialCard({ testimonial }) {
  return (
    <motion.div
      ...
      className="bg-white  text-gray-900 rounded-xl overflow-hidden shadow-xl h-full flex flex-col max-w-md lg:max-w-7xl mx-auto"
    >
      <div className="overflow-hidden rounded-lg  border-gray-200 shadow-sm">
        {/* Grid layout that changes based on breakpoints */}
        <div className="grid lg:grid-cols-5 ">
          {/* Image container - full width on small/medium, 2/5 width on large */}
          <div className="lg:col-span-2 relative h-[350px] w-full sm:h-[360px]">
            <div className="absolute inset-0 z-10" />
            <Image src={testimonial.image || "/placeholder.svg"} alt={testimonial.name} fill className="object-cover" />
          </div>

          {/* Content container - full width on small/medium, 3/5 width on large */}
