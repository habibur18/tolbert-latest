...
//   return (
//     <>
//       {showConfetti && <Confetti width={width} height={height} recycle={false} numberOfPieces={isMobile ? 100 : 200} />}
...
//                           <Input value={otherService} onChange={(e) => setOtherService(e.target.value)} placeholder="Please specify your service needs" className="bg-white/90 rounded-md h-12 !text-lg mt-2 border-0 focus:ring-2 focus:ring-green-500" />
//                         </motion.div>
//                       )}
//                     </motion.div>
//                   )
...
//                 <Button type="button" onClick={() => setStep(2)} disabled={selectedServices.length === 0} className={`w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-400 hover:to-green-500 text-xl h-14 rounded-lg font-semibold shadow-lg ${selectedServices.length === 0 ? "opacity-50 cursor-not-allowed" : "transform transition-transform duration-200"}`}>
...
//                 {/* Message Field */}
//                 <div className="relative">
//                   <textarea id="message" name="message" rows={4} className="block rounded-lg px-4 pb-2.5 pt-5 w-full text-lg text-gray-900 bg-white/90 border-0 appearance-none focus:outline-none focus:ring-2 focus:ring-green-500 peer min-h-[120px] shadow-md resize-y" placeholder=" "></textarea>
        ...
        <AnimatePresence mode="wait">
          {isSubmitted ? (
            ...
            <motion.form key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -100 }} transition={{ duration: 0.3 }} className="space-y-6">
              ...
              <Button type="button" onClick={() => setStep(2)} disabled={selectedServices.length === 0 || (selectedServices.includes("other") && otherService.trim() === "")} className={`w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-400 hover:to-green-500 text-xl h-14 rounded-lg font-semibold shadow-lg ${selectedServices.length === 0 || (selectedServices.includes("other") && otherService.trim() === "") ? "opacity-50 cursor-not-allowed" : "transform transition-transform duration-200"}`}>
                Continue <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
            </motion.form>
          ) : (
